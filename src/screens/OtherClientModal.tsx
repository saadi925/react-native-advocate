import { Avatar, Button, Center, HStack, ScrollView, Text, View } from 'native-base'
...
export default function OtherClientModal({route, navigation} :  any) {
  ...
  navigation.setOptions({
    headerLeft(){
        ...
        <Text color={'white'} fontSize={'xl'}>{data?.displayname || 'unknow user'}</Text>
      </HStack>
    }
  })
  },[navigation])
    ...
    <View  flex={1} bg={COLORS.main}>
       ...
       data && <View mx={2} rounded={'lg'} py={12}>
            ...
            borderWidth={1} rounded={'full'} >
              {sendingRequest ? <ActivityIndicator size={'large'} color={'black'}/> :  <Text color={'black'}>
               Send Request
               </Text>}
